...
#+END_SRC
*** Xcscope
Cscope indexes and then allows for fast navigation of C projects, with limited support for other languages. This is my favorite such tool, because despite being dull and only working well with C, it has a nice interface.
#+BEGIN_SRC emacs-lisp
  (use-package xcscope
   :ensure t
   :init
